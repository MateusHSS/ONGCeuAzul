<?php
  ...
  if($_SESSION['perfil'] == 1){
    $sql="SELECT tabcontribuinte.idcontribuinte, tabcontribuinte.nomecontribuinte, tabendereco.cidade, tabendereco.bairro, tabendereco.rua, tabendereco.numero, tabendereco.referencia, tabendereco.obs, tabendereco.cep, tabcontato.telefone, DATE_FORMAT(tabcontribuinte.datacadastrocontribuinte, '%e/%m/%Y') AS datacad, taboperador.nomeoperador
        ...
        ORDER BY  tabcontribuinte.datacadastrocontribuinte DESC LIMIT 100 OFFSET $inicio";
        ...
        <form method='post' action='../controller/atualContr.php?id=<?php echo $res['idcontribuinte'] ?>'
            ...
            <div class='row'>
                <div class='input-field col l6'>
                    <input id='nome<?php echo $res['idcontribuinte'] ?>' type='text' class='validate' name='nome'
                        value='<?php echo $res['nomecontribuinte'] ?>' required>
                    <label class='active' for='nome<?php echo $res['idcontribuinte'] ?>'>Nome
                        completo</label>
                </div>
                <div class='input-field col l3'>
                    <input id='tel<?php echo $res['idcontribuinte'] ?>' type='text' class='validate tel' name='tel'
                        value='<?php echo $res['telefone'] ?>' required>
                    <label class='active' for='tel'>Telefone</label>
                </div>
                <div class='input-field col l3'>
                    <input id='cep<?php echo $res['idcontribuinte'] ?>' type='text' class='validate cep' name='cep'
                        value='<?php echo $res['cep'] ?>' data-id='<?php echo $res['idcontribuinte'] ?>'>
                    <label class='active' for='cep<?php echo $res['idcontribuinte'] ?>'>CEP</label>
                </div>
            </div>
            <div class='row'>
                <div class='input-field col l3'>
                    <input id='rua<?php echo $res['idcontribuinte'] ?>' type='text' class='validate rua' name='rua'
...
<div class='modal fade' id='nova-doacao<?php echo $res['idcontribuinte'] ?>'>
    <div class='modal-content'>
        ...
        <form method='post' action='../controller/novaDoacao.php?idcontribuinte=<?php echo $res['idcontribuinte'] ?>'
            ...
            <div class="row">
                ...
                <div class="input-field col l3">
                    <input placeholder='mm/aa' id="venc" type="text" class="validate venc" name='venc'>
